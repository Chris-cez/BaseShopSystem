...
func setupClientTestApp() (*fiber.App, *gorm.DB, string) {
	app := fiber.New()
...
func TestCreateClient(t *testing.T) {
	app, db, token := setupClientTestApp()
...
func TestGetClientByID(t *testing.T) {
	app, db, token := setupClientTestApp()
	client := models.Client{Name: "Cliente ID", CPF: "55566677788", AddressID: 1}
	db.Create(&client)
...
func TestUpdateClient(t *testing.T) {
	app, db, token := setupClientTestApp()
	client := models.Client{Name: "Cliente Atualizar", CPF: "99988877766", AddressID: 1}
	db.Create(&client)
...
func TestDeleteClient(t *testing.T) {
	app, db, token := setupClientTestApp()
	client := models.Client{Name: "Cliente Deletar", CPF: "33322211100", AddressID: 1}
	db.Create(&client)
	url := fmt.Sprintf("/api/clients/%d", client.ID)
